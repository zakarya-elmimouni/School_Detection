...
class YOLOObjective:
    def __init__(self, log_path="results/usa/ecp_yolo26n_log_direct_on_small_train_golden.csv"): # change as needed
        self.bounds = np.array([
          [1e-5, 1e-2],     # 0: lr0            (default: 0.001)
          [0.01, 0.1],      # 1: lrf            (default: 0.01)
          [0.9, 0.98],     # 2: momentum       (default: 0.937)
          [1e-4, 0.01],    # 3: weight_decay   (default: 0.0005)
    ...
    def __call__(self, x):
        ...
        save_dir="runs/detect/results/usa/rslt_yolo26n_on_small_train_golden_best_params" # path to saving directory ( change as needed)
...
#            model = YOLO("runs/detect/results/usa/rslt_yolo8n_auto_labeling/exp/weights/best.pt") # existing model pretrained on auto_labeled_data
            ...
            results = model.train(
                ...
                save_period=30,
                verbose=False,
                project="results/usa/rslt_yolo26n_on_small_train_golden_best_params", # path to saving directory ( change as needed)
                name=exp_name,
            )
